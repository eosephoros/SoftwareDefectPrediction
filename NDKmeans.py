...
def startstart(filename0):
    ...
    # csv转无列标题的txt
    def start2(filename):
        data = pd.read_csv('E:/work/personal/stduy/shixun/code/test1/resource/csv/' + filename + '.csv',
                           encoding='utf-8')
        ...
        if os.path.exists('resource/txt/' + filename + '.txt'):
            ...
            print("The file does not exist")
        with open('E:/work/personal/stduy/shixun/code/test1/resource/txt/' + filename + '.txt', 'a+',
                  encoding='utf-8') as f:
...
def start(filename_kmeans):
    ...
    def loadDataSet(fileName):
        data = np.loadtxt("E:/work/personal/stduy/shixun/code/test1/resource/txt/" + fileName + ".txt", delimiter=',')
    ...
    def showCluster(dataSet, k, centroids, clusterAssment, filename):
        ...
        if (len(pointsInCluster) < 20):
            raise RuntimeError('testError')
        # 先删除再添加
        if os.path.exists('E:/work/personal/stduy/shixun/code/test1/resource/result/' + filename + "result" + '.txt'):
            os.remove('E:/work/personal/stduy/shixun/code/test1/resource/result/' + filename + "result" + '.txt')
        else:
            print("The file does not exist")
        np.savetxt('E:/work/personal/stduy/shixun/code/test1/resource/result/' + filename + "result" + ".txt",
                   pointsInCluster, fmt='%1.2f')
...
def chulitxtwenjianzhunbei(filenameforall2):
    ...
    updateFile("E:/work/personal/stduy/shixun/code/test1/resource/result/" + filenameforall2 + "result.txt", ' ', ',')

    origdata = pd.read_csv("E:/work/personal/stduy/shixun/code/test1/resource/csv/" + filenameforall2 + ".csv")
    ...
    df_example_noCols = pd.read_csv(
        "E:/work/personal/stduy/shixun/code/test1/resource/result/" + filenameforall2 + "result.txt", header=None)
    df_example_noCols.columns = k
    df1 = df_example_noCols
    df1.to_csv('E:/work/personal/stduy/shixun/code/test1/resource/KMeansResultCSV/' + filenameforall2 + 'result.csv',
               index=None)
    print("转换完成！")


os.chdir("E:\\work\\personal\\stduy\\shixun\code\\test1\\resource\\arff")
...
# filePath = 'E:\\work\\personal\\stduy\\shixun\code\\test1\\resource'
...
for i in range(0, len(names)):
    if i >= 6:
        ...
        start(names[i])
        time.sleep(5)
        chulitxtwenjianzhunbei(names[i])
